...
-(void)initBlock{
    ...
    self.blockOne = ^{
           NSLog(@"blockOne无参数，无返回值");
       };
       
    self.blockTow = ^(int value) {
        NSLog(@"blockTow有参数===%d，无返回值",value);
    };
    
    self.blockThree = ^NSString * _Nullable{
        NSLog(@"blockThree无参数，有返回值");
        return @"100";
    };
    
    self.blockFour = ^NSString * _Nullable(int value) {
        NSLog(@"blockFour有参数===%d，有返回值",value);
        return @"200";
    };
...
-(void)initView{
    ...
    NSLog(@"blockThree返回值===%@",value);
    NSLog(@"blockFour返回值===%@",value2);
}
